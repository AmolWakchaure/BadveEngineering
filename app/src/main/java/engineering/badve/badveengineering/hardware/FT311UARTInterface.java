...
package engineering.badve.badveengineering.hardware;
...
public class FT311UARTInterface extends Activity {
	...
	private static final String ACTION_USB_PERMISSION = Constants.HARWARE_INTERFACE+".USB_PERMISSION";
	...
	// private SharedPreferences prefs;
	// // editor for storing required data in to SharedPreferences
	// private SharedPreferences.Editor editor;
	...
	public FT311UARTInterface(Context context, SharedPreferences sharePrefSettings)
	{
		...
		//BeplLog.d("LED", "usbmanager" +usbmanager);
		...
		prefs = context.getSharedPreferences("com.snsystems", Context.MODE_PRIVATE);
	...
	/* method to send on USB */
	public void SendPacket(int numBytes, byte[] buffer) {
		try {
			if (outputstream != null) {
				outputstream.write(buffer, 0, numBytes);
				String message = new String(writeusbdata);
				// Toast.makeText(global_context, "message " + message,
	...
	public int ResumeAccessory() {
		...
		if (accessories != null) {
			...
			// BeplLog.e(">>@@","ResumeAccessory RETURN 2 (accessories == null)");
	...
	private final BroadcastReceiver mUsbReceiver = new BroadcastReceiver()
	{
		...
		public void onReceive(Context context, Intent intent) {
			...
			else if (UsbManager.ACTION_USB_ACCESSORY_DETACHED.equals(action))
			{
				...
				// Toast.makeText(global_context, "dstroy", Toast.LENGTH_LONG)
				// .show();
				CloseAccessory();
			}
			else
	...
	public void onScreen()
	{
		try
		{
			PowerManager pm = (PowerManager) global_context.getSystemService(Context.POWER_SERVICE);
			boolean isScreenOn = pm.isScreenOn();
			if (isScreenOn == false)
			{
				WakeLock wakeLock = pm.newWakeLock(PowerManager.FULL_WAKE_LOCK | PowerManager.ACQUIRE_CAUSES_WAKEUP, "MyWakeLock");
				wakeLock.acquire(10000);
			}
		}
		catch (Exception e)
		{
			// Toast.makeText(global_context, "error4",
			// Toast.LENGTH_LONG).show();
		}
	}
